...
### Day 1：范围冻结与工单拆分
- 锁定首周目标：战斗-掉落-背包可玩闭环。
- 完成模块模板填写并拆分任务（数据/服务/UI/事件/测试）。

### Day 2：掉落系统接入
- 基于 DeathEvent 接入掉落流程。
- 完成掉落配置与最小掉落算法。

### Day 3：背包系统 MVP
- 完成 InventoryData + InventoryService + InventoryPanel。
- 打通掉落入包与存档还原。

### Day 4：AB 包产线
- 完成资源分组（UI/Monster/Map/Config）与 AB 构建脚本。
- 验证 AB 加载、版本号与依赖关系。

### Day 5：手机打包
- 完成 Android 打包（先 APK 后 AAB）。
- 真机验证黄金链路与性能基线。

### Day 6：回归与修复
- 跑 EditMode + PlayMode smoke。
- 修复阻断问题并更新已知问题列表。

### Day 7：发布与复盘
- 发布周版本（含 AB 版本号）。
- 输出复盘结论与下一周迭代计划（技能/成长/装备）。
<<<<<<< ours
>>>>>>> theirs
=======
>>>>>>> theirs
